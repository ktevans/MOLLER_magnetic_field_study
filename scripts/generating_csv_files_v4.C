...
void generating_csv_files_v4(string infile, const int cut = 0){
  ...
  string fileName_merged = pathName + fieldMap + "_" + passName + "_" + targetName + "_merged.csv";
  csvFile_merged.open(fileName_merged, std::ofstream::app);

  for(int ihole = 0; ihole < nholes; ihole++){

    std::ofstream csvFile;
    string fileName = pathName + fieldMap + "_" + passName + "_" + targetName + "_" + holeNames[ihole] + ".csv";
    csvFile.open(fileName, std::ofstream::app);
    csvFile << "tg_th" << "," << "tg_ph" << "," << "tg_p" << "," << "tg_vz" << "," << "sieve_r" << "," << "gem_r" << "," << "r_prime" << "," << "gem_ph" << "," << "phi_prime" << "," << "phi_local" << endl;
    ...
    for(int j=0; j<nevents; j++){
      ...
      if(infile.find("Optics1") != std::string::npos) tg_vz = -5124.5;
      else if(infile.find("Optics2") != std::string::npos) tg_vz = -3875.5;
      else if(infile.find("Optics3") != std::string::npos) tg_vz = -4499.873;
      ...
      if( IsInside(ellipse_rphi, gem_r, gem_ph) && IsInside(ellipse_rrprime, r_prime, gem_ph) && IsInside(ellipse_phi_phiprime, phi_prime, gem_ph) ){
        ...
        csvFile << tg_th << "," << tg_ph << "," << tg_p << "," << tg_vz << "," << sieve_r << "," << gem_r << "," << r_prime << "," << gem_ph << "," << phi_prime << "," << phi_local << endl;
        csvFile_merged << tg_th << "\t" << tg_ph << "\t" << tg_p << "\t" << tg_vz << "\t" << sieve_r << "\t" << gem_r << "\t" << r_prime << "\t" << gem_ph << "\t" << phi_prime << "\t" << phi_local << endl;
  ...
  string outfileName = pathName + fieldMap + "_" + passName + "_" + targetName + "_plots.root";
  if(cut) outfileName = pathName + fieldMap + "_" + passName + "_" + targetName + "_plots_non_radiative.root";
