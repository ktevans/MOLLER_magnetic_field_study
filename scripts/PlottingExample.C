...
void PlottingExample(const TString& files)
{
  ...
  //TH2* h2 = new TH2F("h2", "h2 title", 40, 0.0, 2.0, 30, -1.5, 3.5);
   ...
   //[2D hist]* [name] = new ["TH2F" means one float per bin]("name","title of hist", [# of x bins], [x_min], [x_max], [# of y bins],
[y_min], [y_max]);
  ...
  for (size_t iev = 0; iev < T->GetEntries(); iev++) 
  {
    ...
    for (size_t ihit = 0; ihit < hits->size(); ihit++) 
    {
      ...
      {
        det28_prim_xy -> Fill(hit.x,hit.y,rate); //"rate" weights the histogram properly
...
//Now, you can place each stat box where you want it. Here I am just moving them in the Y direction, but you could make them wider too. 
//The histogram is plotted between (0.1,0.1) and (0.9,0.9) in NDC. (0,0) is the bottom left of the canvas, and (1,1) is the top right.

TPaveStats *s1 = (TPaveStats*)maindet_energy_500_below->GetListOfFunctions()->FindObject("stats");
...
//h(1/2/3)color is the variable used in the dummy histograms to label the fill color for those histograms. 
//The final spot in AddEntry, "f", means we want this entry in the histogram to be a box filled with the fill color. 
// We can call the same color that the energy histogram markers are from the dummy histograms.
