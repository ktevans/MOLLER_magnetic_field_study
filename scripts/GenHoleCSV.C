...
void GenHoleCSV(string infile, double rotation, const int cut = 0)
{
  ...
  if(rotation == 51.0)
  {
    ...
    //holes = {"21", "22", "23", "31", "32", "33", "41", "42", "43", "51", "52", "53", "61", "62", "63", "71", "72", "73", "11", "12", "13"};
  ...
  else if(infile.find("Pass5") != std::string::npos)
  {
    ...
    for(int j=0; j<nholes; j++){ radial[j] = new TH1D(Form("r_%s", holeNames[j].c_str()),"gem_r_distribution;r[mm];Counts",800, 650, 1050); }
  ...
  for(int j=0; j<nevents; j++)
  {
    ...
    if(index_hole!=-999)
    {
      ...
      cout << "hole " << index_hole << " is done." << endl;
    }
  ...
  for(int ihole = 0; ihole < nholes; ihole++)
  {
    ...
    if (radial[ihole])
    {
      ...
      // Fit gaussians iteratively???????? Just fit it 5 times?
      for(int itr = 0; itr < 5; itr++)
      {

        radial[ihole]->Fit(func, "RQN","",func->GetParameter(1)-2.0*func->GetParameter(2),func->GetParameter(1)+2.0*func->GetParameter(2));
      ...
      for(int itr = 0; itr < 5; itr++)
      {
        ...
        rprime[ihole]->Fit(func2, "RQN","",func2->GetParameter(1)-2.0*func2->GetParameter(2),func2->GetParameter(1)+2.0*func2->GetParameter(2));
      ...
      // Use the gaussian parameters to define bounds on our variables.
